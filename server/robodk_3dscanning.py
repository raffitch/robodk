...
tooldraw = RDK.Item('camera')
robot.setPoseFrame(framedraw)
robot.setPoseTool(tooldraw)
RDK.setRunMode(6)
...
def visit_targets_and_collect_data():
    ...
    for target_name in target_names:
        ...
        # Move to the target (this assumes you have a robot item defined. Adjust accordingly)
        robot.MoveJ(target)
...
def save_point_cloud(all_depth_data, all_color_data, all_transformations, cam_mtx):
    ...
    mesh_poisson, densities = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(object_only_pcd,
    ...
    o3d.visualization.draw_geometries([object_only_pcd, mesh_poisson] if O3D_DISPLAY_POINTS else [mesh_poisson],
                                      mesh_show_wireframe=O3D_DISPLAY_WIREFRAME, mesh_show_back_face=True, width=1280,
                                      height=731, left=0, top=720)
    ...
    o3d.visualization.draw_geometries([mesh_poisson], mesh_show_wireframe=O3D_DISPLAY_WIREFRAME,
                                      mesh_show_back_face=True, width=1280, height=731, left=0, top=720)
