...
def handle_frame(bigDepth, bigColor, marker_3d_positions, marker_rotations, marker_translations, corners_marker):
    ...
    for position, rotation, translation, marker_corners in zip(marker_3d_positions, marker_rotations,
                                                               marker_translations, corners_marker):
        rotation_n = tuple(rotation[0])
        rotation_n = (0, 0, 0)
        T_CM = rm.TxyzRxyz_2_Pose(position + rotation_n)
        T_RM = T_RB * T_TC * T_CM
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        try:
            ...
            if bigDepth is not None and bigColor is not None:
                marker_3d_positions, marker_rotations, marker_translations, corners_marker = process_markers(bigDepth,
                                                                                                             bigColor)
                handle_frame(bigDepth, bigColor, marker_3d_positions, marker_rotations, marker_translations,
                             corners_marker)
