...
def summarize_distance_trial(distance_mm, plane_point_sets, length_samples,
    ...
    trial's ``length_err_mm`` is the WORST (max) of the per-sample errors, so a
    trial cannot pass by only reporting its best known-length measurement.
    """
...
def choose_dstar(trials, *, max_rms_mm, max_height_repeat_mm, max_normal_repeat_deg,
    ...
    def _passes(t: "DistanceTrial") -> bool:
        metrics = (t.plane_rms_mm, t.height_repeat_mm, t.normal_repeat_deg,
                  t.length_err_mm, t.coverage_frac)
